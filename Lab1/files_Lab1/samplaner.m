...
ut = zeros(uint8(rad/2), uint8(kolumn/2));
...
% the column number is an odd integer (for example kvarn_udda.tif). 
% 
% Explain how your code handle images whos number of rows/columns are odd: (answer here as a comment).
% OUR ANSWER: By adding the "-1" to the line 71 & 71 "for i = 1:2:rad-1" and 
% "for j= 1:2:kolumn-1", we can increment by two for each loop until the end, and if the
% original image has an odd dimension this "rad/column -1" makes sure that we dont exceed the array bounds. 
% If the image has an even dimension, the last row/column will simply not be processed, but since it is just
% one row/column there is no issue with this. 
%
